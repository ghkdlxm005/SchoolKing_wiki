...
있어야 정상인 것이 없으면 조용히 넘기지 말고 `warn`을 남긴다.

```lua
if not _sfx then warn("[Compass] SFX.compass_sound 없음"); return end
```

선택적인 것(있으면 좋고 없어도 되는 것)만 조용히 넘긴다. 둘을 구분하는 게 관건이다.
...
실제로 맵 작업자가 관중을 다시 묶자마자 이름 기준 제외가 무너져 윤곽선 버그가 재발했다.

### 4. 있어야 할 것이 다 있는지 전수 점검한다
