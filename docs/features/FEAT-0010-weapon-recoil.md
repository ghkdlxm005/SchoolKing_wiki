...
- 수직 전용: 발사마다 위쪽 kick(`RecoilVertical`)을 실제 조준 pitch에 그대로 누적한다. 좌우 반동은 사용하지 않는다.
- 회복 없음: 누적된 pitch는 되돌아오지 않는다. 화면이 튕겼다 돌아오는 흔들림이 아니라 조준점이 계속 위로 올라가고, 그만큼 직접 내려서 제어한다.
- 연사 증가(progressive): 연속 사격 시 `RecoilProgressive`만큼 발당 추가한다(스트릭 최대 `RecoilProgressiveMax`). 발사를 멈추면(`RecoilStreakResetTime`) 리셋된다.
- 조준 시 감소: ADS 중이면 `AimRecoilMultiplier`로 완화한다.
- 전체 세기는 GunConfig 무기별 값의 30%로 튜닝했다(과하다는 피드백 반영).
...
- "실제 반동"(조준 이동) 방식이라 화면 흔들림이 없다. 세기는 GunConfig에서 조절한다.
