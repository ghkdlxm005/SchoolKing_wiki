...
무기 `ClientHandler`는 Tool 안에 들어 있는 LocalScript다. 뷰모델 정리는 두 시점에만 걸려 있었다.
...
1. `Died`가 발생하지 않는다. 캐릭터가 죽은 게 아니라 파괴되기 때문이다.
2. 정리할 주체가 사라진다. 캐릭터가 파괴되면 그 안의 Tool과 `ClientHandler` 스크립트도 함께 삭제된다. 정리 코드를 실행할 스크립트 자체가 없어진다.
...
정리 시점을 스크립트가 아직 살아 있는 동안으로 앞당기고, 안전망을 하나 더 뒀다.
...
라운드 리셋·리스폰 후 뷰모델이 남지 않는다. 재발 방지를 위해 정리 주체와 시점을 위치에 맞춘다.
...
Tool 안의 스크립트는 캐릭터와 함께 삭제되므로 자기 뒷정리를 스스로 보장할 수 없다. `Humanoid.Died`만 믿으면 안 된다. `LoadCharacter`·강제 삭제·팀 변경 등은 죽음 없이 캐릭터를 교체한다.
