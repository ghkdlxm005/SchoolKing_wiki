...
관중석마다 적 윤곽선이 뜨는 곳과 안 뜨는 곳이 갈렸고, 배경 함성도 일부 관중석에서만 났다.

## 원인

관중이 9개 구역으로 흩어져 있는데, 코드는 그중 일부만 보고 있었다.
...
| 구역 | 관중 수 | `NoOutline` |
| --- | --- | --- |
| `audience A` ×2, `B` ×2, `Back` ×1 | 541 | 제외됨 |
| `Model` 스탠드 ×4 (맵 네 귀퉁이) | 654 | 누락 |
| 낱개 관중 (남쪽 계단식 관람석) | 145 | 누락 |
...
1. 윤곽선 — 관중 스탠드와 낱개 관중을 전수 탐색해 `NoOutline`을 태그했다. 판별 기준은 두 가지를 모두 만족하는 모델이다.
   - `Humanoid`를 포함
   - 관중용 스크립트(`Dance` / `LoopJump` / `Health` / `HealthScript v2.0` / `RobloxTeam` / `Animate`)만 있고 그 외 스크립트가 없음

   낱개는 관람석 구역(고지대) 좌표 범위로 한정해 필드의 테스트 더미가 섞이지 않게 했다.

2. 함성 — 같은 이름의 관중석을 전부 수집하도록 변경했다.
...
윤곽선 대상 비플레이어 휴머노이드가 0명이 됐고, 9개 관중석 전부에서 함성이 난다.

후속으로 맵 작업자가 낱개 관중 145명을 `Model` 하나로 묶으면서 새 그룹에 `NoOutline`이 없어 증상이 재발했고, 태그를 다시 적용했다. 맵 구조가 바뀌면 태그가 유실되므로, 그룹핑이 확정되면 관중 전용 폴더 하나로 모으고 그 폴더에만 태그하는 방식이 안전하다.

재발 방지 차원에서 이름으로 찾을 때 동명 인스턴스가 여럿일 수 있다고 가정하고, 제외 목록을 이름으로 하드코딩하지 않고 특성(스크립트 구성·좌표)으로 판별한다.
