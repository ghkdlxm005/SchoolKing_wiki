...
로블록스 측에서 해당 오디오 에셋이 보관(archived) 상태로 전환되어 로드가 거부됐다. 업로더가 직접 보관했거나 심의로 내려간 경우다.

이 두 개는 이번 작업에서 건드린 사운드가 아니다. 기존부터 있던 항목이며, 에셋 상태 변화로 뒤늦게 드러났다.

## 조치

보관 해제(Unarchive)로 복구했다.
...
해결됐다. 2026-07-19 전수 점검에서 두 에셋 모두 정상 로드를 확인했고, 게임 전체 사운드 59개에서 로드 실패 0건이었다.

재발 방지 차원에서 툴박스에서 가져온 오디오는 언제든 내려갈 수 있다고 가정하고, 핵심 피드백음(명중·처치)은 가능하면 직접 업로드한 에셋을 쓴다. 콘솔의 `Failed to load sound` 경고는 무시하지 않는다. 소리가 없다는 것은 전투 피드백이 통째로 빠졌다는 뜻이다.
...
- 같은 원인(에셋 보관)으로 컴퍼스 사운드 2종도 죽어 있었다. → [BUG-0013](./BUG-0013-compass-marker-sound-path.md)
