...
이를 조사하다 더 큰 문제가 드러났다. 애니메이션 마커에 연결된 컴퍼스 사운드 13개가 한 번도 재생된 적이 없었다. `shot_1`, `reload_all`, `clink`, `gear`, `spin` 전부다. 에러조차 나지 않아 소리가 원래 그런 줄 알고 넘어가고 있었다.
...
실제 위치는 `workspace.SFX.compass_sound`인데 워크스페이스 최상위에서 찾고 있었다. `_sfx`가 `nil`이 되고, 바로 아래 `if _sfx then` 가드가 바인딩 블록 전체를 조용히 건너뛰었다. nil 가드가 크래시를 막아준 대신, 기능이 통째로 빠진 사실도 함께 숨겼다.
...
2. 바인딩을 `playAnim`으로 이동 — `bindSfxMarkers(track)`를 공통 함수로 빼고 재생 직전에 호출. `Fire`를 포함한 모든 트랙에 적용되며, 새 애니를 추가해도 자동으로 걸린다.
3. 옛 경로 제거
...
컴퍼스 마커 사운드 13개가 모든 트랙에서 재생된다. 게임 전체 사운드 59개 로드 실패 0건을 확인했다.

남은 부작용: 컴퍼스 사운드는 이제 본인에게만 들린다. 다른 플레이어용 3D 재생 경로가 없어졌다. 필요해지면 마커 재생을 서버 브로드캐스트로 확장해야 한다.
...
- [LESSON-0005](../lessons/LESSON-0005-silent-failure.md) — 에러가 없다고 정상은 아니다. nil 가드로 감싼 선택적 기능은 실패해도 조용하다.
