...
- **공유 `DamageNumbers` 하나로 통합**: 무기별 표시 제거, 하나의 클라 스크립트가 `HitFeedbackEvent`를 단독 처리(히트마커 포함).
- **합산 표기**: 같은 대상에 짧은 시간(0.7초) 내 명중은 우측에 **누적 합산**, 단일 값은 좌측에 **부위별 색**(머리 노랑/몸통 흰/팔다리 회색).
- **투사체 표시**: 캔 폭발·컵 파편 데미지도 `HitFeedbackEvent`를 발사(소유자를 던질 때 캡처해 전달) → 숫자 표시 + 벽 투시.
