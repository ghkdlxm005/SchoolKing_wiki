...
`CFrame.new(위치)`는 **회전이 항등(identity)** 인 CFrame을 만든다. 즉 월드 −Z를 보는 기본 방향으로 고정된다. `SpawnLocation`에 작성자가 지정해 둔 방향은 전달 경로 자체가 없었다.

로블록스 기본 스폰은 SpawnLocation의 `LookVector` 방향으로 캐릭터를 세우는데, 커스텀 배치 로직이 그 동작을 덮어쓰면서 방향 정보만 유실된 것이다.
...
스폰 지점의 `LookVector`를 살려서 배치하도록 두 곳 모두 수정.
...
- **수평 성분만** 사용 — 스폰 파트가 기울어져 있어도 캐릭터는 똑바로 선다.
- 수평 성분이 0에 수렴하면(스폰이 완전히 수직) `−Z` 기본값으로 폴백.

## 재발 방지

**캐릭터를 옮길 때 `CFrame.new(위치)`를 쓰지 않는다.** 방향이 의미 있는 배치라면 원본 CFrame의 회전을 반드시 함께 넘긴다.
...
또한 **같은 배치 로직이 두 파일에 복제**되어 있어 한 곳만 고치면 절반만 해결된다. 스폰 배치는 향후 한 곳으로 합치는 것이 좋다.
