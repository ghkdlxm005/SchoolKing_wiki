...
정상 클라이언트에서는 문제가 없다. 조작된 클라이언트에서만 발생한다. 서버 발사 검증에 구멍이 세 곳 있다.
...
맵 반대편 좌표를 `origin`으로 보내면 서버가 그 지점에서 레이캐스트한다. 벽 뒤·맵 밖에서 사격이 가능하다.
...
스프레드(반동에 의한 탄 퍼짐)를 클라이언트가 계산해서 최종 방향을 보낸다. 서버는 길이만 본다. 스프레드를 0으로 만들어 무반동 사격이 가능하고, 카메라가 어디를 보든 임의의 대상에게 직접 조준할 수 있다.

### ③ 토스터 펠릿 배열에 개수 상한이 없음 — critical

샷건인 토스터는 펠릿 여러 발이므로 `direction`을 배열로 받는다.
...
배열에 펠릿 500개를 넣어 보내면 탄약 1발로 500회 데미지가 들어간다. 즉사 수준이고 셋 중 가장 시급하다.
...
서버가 직접 레이캐스트하므로 "맞았다"는 클라 주장은 신뢰하지 않는다. 이 부분은 올바르다. 문제는 판정에 쓰이는 입력값 자체가 클라이언트 제공값인데 서버가 이를 검증하지 않는다는 점이다. 서버가 신뢰한 것은 "맞았다"가 아니라 "어디서, 어느 방향으로, 몇 발"이었다.

또한 4개 무기 핸들러가 거의 동일한 코드를 복사해서 쓰고 있어, 검증을 추가하려면 4곳을 모두 고쳐야 한다.

## 조치

제안만 정리했고 아직 적용하지 않았다.
...
근본적으로는 4개 무기의 공통 발사 검증을 모듈 하나로 합치는 편이 맞다. 지금 구조에서는 검증 하나 추가할 때마다 4곳을 동일하게 고쳐야 하고, 한 곳을 빠뜨리면 그 무기만 뚫린다.

## 결과

미해결(open). 우선순위 협의 대기 중이다. 세 구멍 모두 코드 반영 전이다.

서버 권위는 "판정을 서버가 한다"로 끝나지 않는다. 판정의 입력값(위치·방향·개수·배열 길이)도 검증해야 하며, 클라이언트에서 오는 값은 전부 공격자 제어 하에 있다고 가정한다.
